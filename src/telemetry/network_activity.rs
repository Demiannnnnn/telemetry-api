...
//!
//! Handlers for ingesting, querying, and managing encrypted network activity telemetry.
...
/// Ingests a batch of encrypted network activity telemetry records.
pub async fn ingest_network_activity(
    ...
    // Check worker active state and consent
    let consent_check = sqlx::query!(
        r#"
        SELECT is_active, consent_flags->>'network_activity' as "has_consent"
        FROM workers 
    ...
    if consent_check.has_consent.as_deref() != Some("true") {
        return Err(AppError::Forbidden(
            "Worker has not given consent for network_activity telemetry".to_string(),
        ));
    }

    // Validate timestamp skew (max 5 minutes into the future)
    let now = Utc::now();
    ...
    for record in payload.records {
        ...
        sqlx::query!(
            r#"
            INSERT INTO telemetry_network_activities (
    ...
    tracing::info!(
        ...
        records_count = count,
        batch_id = %batch_id,
        "Network activity telemetry batch ingested"
    );
...
/// Queries encrypted network activity telemetry for a worker (administrator only).
pub async fn query_network_activity(
    ...
    let total = sqlx::query_scalar!(
        r#"
        SELECT COUNT(*)
        FROM telemetry_network_activities
        WHERE worker_id = $1
...
/// Retrieves a specific network activity record by ID (administrator only).
...
/// Deletes a specific network activity record (administrator only).
pub async fn delete_network_activity(
    RequireAdmin(admin): RequireAdmin,
    Path(record_id): Path<Uuid>,
    State(state): State<Arc<AppState>>,
) -> Result<StatusCode, AppError> {
    let row = sqlx::query!(
        "SELECT worker_id FROM telemetry_network_activities WHERE id = $1",
        record_id
    )
    .fetch_optional(&state.db)
    .await
    ...
    sqlx::query!(
        "DELETE FROM telemetry_network_activities WHERE id = $1",
        record_id
    )
    .execute(&state.db)
    .await
