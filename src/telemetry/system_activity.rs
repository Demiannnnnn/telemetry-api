...
//!
//! Handlers for ingesting, querying, and managing encrypted system activity telemetry.
...
/// Ingests a batch of encrypted system activity telemetry records.
pub async fn ingest_system_activity(
    ...
    if consent_check.has_consent.as_deref() != Some("true") {
        return Err(AppError::Forbidden(
            "Worker has not given consent for system_activity telemetry".to_string(),
        ));
    }
    ...
    tracing::info!(
        ...
        records_count = count,
        batch_id = %batch_id,
        "System activity telemetry batch ingested"
    );
...
/// Queries encrypted system activity telemetry for a worker (administrator only).
pub async fn query_system_activity(
    ...
    let total = sqlx::query_scalar!(
        r#"
        SELECT COUNT(*)
        FROM telemetry_system_activities
        WHERE worker_id = $1
...
/// Retrieves a specific system activity record by ID (administrator only).
...
/// Deletes a specific system activity record (administrator only).
pub async fn delete_system_activity(
    RequireAdmin(admin): RequireAdmin,
    Path(record_id): Path<Uuid>,
    State(state): State<Arc<AppState>>,
) -> Result<StatusCode, AppError> {
    let row = sqlx::query!(
        "SELECT worker_id FROM telemetry_system_activities WHERE id = $1",
        record_id
    )
    .fetch_optional(&state.db)
    .await
    ...
    sqlx::query!(
        "DELETE FROM telemetry_system_activities WHERE id = $1",
        record_id
    )
    .execute(&state.db)
    .await
